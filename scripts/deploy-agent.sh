...
#   --ca-service-url <url>     CA Lambda Service URL (required for dynamic certificates)
        ...
        case "$key" in
            ...
            local_proxy_port)
             ...
             kill_endpoint)
                 ...
                 ;;
            ca_service_url)
                [[ -z "$CA_SERVICE_URL" ]] && CA_SERVICE_URL="$value"
                ;;
            log_level)
...
ca_cert_file: "$INSTALL_PATH/ca.crt"
...
copy_certificates_to_installation() {
    log_substep "Setting up Certificate Directory"

    # Create certs subdirectory for CA certificate
    local certs_dir="$INSTALL_PATH/certs"
    mkdir -p "$certs_dir"

    log_info "Certificate directory ready: $certs_dir"
    log_info "Dynamic certificates will be generated by CA Lambda at runtime"
}
